...
def main() -> None:
    ...
    version = tuple(int(part) for part in str(meta.get("version", "0.0.0")).split("."))
    if version < (0, 2, 2) or int(meta.get("build", 0)) < 178:
        fail("current release predates the V0.2.2+178 contract")
