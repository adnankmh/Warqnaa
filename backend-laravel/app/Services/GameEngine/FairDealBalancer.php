...
final class FairDealBalancer
{
    ...
    public static function balance(array $hands, string $mode = 'trick'): array
    {
        ...
        $premiumQuota = match ($mode) {
            'capture', 'light', 'jackaroo', 'action' => $handSize >= 4 ? 1 : 0,
            default => $handSize >= 13 ? 2 : ($handSize >= 7 ? 1 : 0),
        };
        $maxSpread = match ($mode) {
            'rummy', 'hand', 'banakil' => max(18, (int) round($handSize * 2.0)),
            'baloot' => 16,
            'capture', 'light' => max(8, (int) round($handSize * 1.6)),
            'jackaroo', 'action' => max(10, (int) round($handSize * 1.8)),
        ...
        // Never return a corrupted deal, even if a future card format is added.
        if (!self::sameMultiset($original, $hands) || !self::sameHandSizes($original, $hands)) {
            return $original;
        }
    ...
    /** @param array<string|int,array<int,string>> $before @param array<string|int,array<int,string>> $after */
    private static function sameHandSizes(array $before, array $after): bool
    {
        if (array_keys($before) !== array_keys($after)) return false;
        foreach ($before as $key => $hand) {
            if (count($hand) !== count($after[$key] ?? [])) return false;
        }
        return true;
    }

    /** @param array<string|int,array<int,string>> $before @param array<string|int,array<int,string>> $after */
    private static function sameMultiset(array $before, array $after): bool
    {
        $counts = static function (array $hands): array {
            $out = [];
            foreach ($hands as $hand) foreach ($hand as $card) $out[$card] = ($out[$card] ?? 0) + 1;
            ksort($out);
            return $out;
        };
    ...
    private static function cardScore(string $card, string $mode): int
    {
        ...
        if (in_array($mode, ['jackaroo', 'action'], true)) {
            return match ($rank) {
                'A' => 19, 'K' => 18, 'J' => 17, '7' => 16, '4' => 14,
                'Q' => 12, '10' => 10, default => max(2, $value),
            };
        }
