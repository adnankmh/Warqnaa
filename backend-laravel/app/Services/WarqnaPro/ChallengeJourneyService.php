...
class ChallengeJourneyService
{
    ...
    public function start(User $user, string $gameKey, int $stages): array
    {
        ...
        return DB::transaction(function () use ($user, $gameKey, $stages) {
            ...
            [$opponentId,$opponentName] = $this->pickOpponent($user);
    ...
    /** @return array<string,mixed> */
    public function record(User $user, bool $won, string $clientResultId, string $gameKey): array
    {
        return DB::transaction(function () use ($user,$won,$clientResultId,$gameKey) {
    ...
    private function pickOpponent(User $user): array
    {
        ...
        $bots=['Adnan','Bayan','Kenan','Jameel','Raad','Asem','Moatasem','Hossam','Janan','Hoor','Jannat','Alaa','Afnan','Shahd','Hala','Shatha','Qamar'];
