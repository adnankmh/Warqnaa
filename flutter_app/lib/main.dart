...
class _TarneebRoomPageState extends State<TarneebRoomPage> {
    ...
    try {
      ...
      if (awayMode) {
        ...
        if (autoPlayedTurns >= 3) {
          ...
          if (mounted) {
            showToast(context, trV025(widget.controller, 'absenceEjected'));
  ...
  void _maybeRewardWin() {
    ...
    rewardGranted = true;
    if (!awayMode) {
      unawaited(widget.controller.recordChallengeJourneyResultV025(won: engine.winnerTeam == 0, gameId: 'tarneeb'));
    }
...
class _ServerEngineRoomPageState extends State<ServerEngineRoomPage> with WidgetsBindingObserver {
  ...
  void _reportChallengeResultV025(Map<String, dynamic> roomPayload) {
    ...
    unawaited(widget.controller.recordChallengeJourneyResultV025(won: won, gameId: widget.game.id));
  }
...
class _AdminDashboardPageState extends State<AdminDashboardPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _adminPlayerActionV025(int userId, String action) async {
    ...
    try {
      await widget.controller.api.adminUserAction(userId, action, amount: action == 'grant_tokens' ? '200' : null);
      if (!mounted) return;
