...
class FairDealBalancer {
  ...
  static List<List<String>> balanceCodes(List<List<String>> source, {String mode = 'trick'}) {
    final original = source.map((hand) => List<String>.from(hand)).toList();
    ...
    final premiumQuota = const <String>{'capture', 'light', 'jackaroo', 'action'}.contains(mode)
        ? (handSize >= 4 ? 1 : 0)
        : handSize >= 13
            ? 2
            : handSize >= 7
    ...
    final maxSpread = mode == 'rummy'
        ...
        : mode == 'baloot'
            ? 16
            : const <String>{'capture', 'light'}.contains(mode)
                ? (handSize * 1.6).round().clamp(8, 40)
                : const <String>{'jackaroo', 'action'}.contains(mode)
                    ? (handSize * 1.8).round().clamp(10, 50)
    ...
    if (!_sameHandSizes(original, hands) || !_sameMultiset(original, hands)) return original;
    return hands;
  }

  static bool _sameHandSizes(List<List<String>> before, List<List<String>> after) {
    if (before.length != after.length) return false;
    for (var i = 0; i < before.length; i++) {
      if (before[i].length != after[i].length) return false;
    }
    return true;
  }

  static bool _sameMultiset(List<List<String>> before, List<List<String>> after) {
    Map<String, int> counts(List<List<String>> hands) {
    ...
    final left = counts(before);
    final right = counts(after);
  ...
  static int _cardScore(String card, String mode) {
    ...
    if (const <String>{'jackaroo', 'action'}.contains(mode)) {
      return switch (rank) {
        'A' => 19,
        'K' => 18,
        'J' => 17,
