...
const Map<String, Map<String, String>> _v025Text = {
  'ar': {
    ...
    'adminOnly': 'المصمم الشامل متاح للمدير الرئيسي Adnan فقط.', 'sendTokens': 'إرسال 200 توكن',
  ...
  'en': {
    ...
    'adminOnly': 'The comprehensive designer is available only to the primary Adnan administrator.', 'sendTokens': 'Send 200 tokens',
  ...
  'de': {
    ...
    'adminOnly': 'Der vollständige Designer ist nur für den Hauptadministrator Adnan verfügbar.', 'sendTokens': '200 Token senden',
  ...
  'tr': {
    ...
    'adminOnly': 'Kapsamlı tasarımcı yalnızca ana yönetici Adnan içindir.', 'sendTokens': '200 jeton gönder',
  ...
  'fr': {
    ...
    'adminOnly': 'Le designer complet est réservé à l’administrateur principal Adnan.', 'sendTokens': 'Envoyer 200 jetons',
  ...
  'es': {
    ...
    'adminOnly': 'El diseñador completo es exclusivo del administrador principal Adnan.', 'sendTokens': 'Enviar 200 fichas',
...
String trV025(AppController controller, String key) =>
    _v025Text[controller.localeCode]?[key] ?? _v025Text['en']?[key] ?? key;
...
String rewardLabelV025(AppController controller, Map<String, dynamic> reward) {
  ...
  return switch (reward['type']?.toString()) {
    'pasha_days' => '$amount Pasha day${amount == 1 ? '' : 's'}',
    'table_days' => '$amount-day table',
    'prize_box' => 'Prize box',
    'ticket' => 'Competition ticket 200',
    'booster_hours' => '$amount-hour booster',
    'chat_color_days' => '$amount-day text color',
    'name_color_days' => '$amount-day player color',
    'tokens' => '$amount tokens',
    _ => trV025(controller, 'reward'),
  };
}
...
extension WarqnaV025Controller on AppController {
  ...
  Future<Map<String, dynamic>?> recordChallengeJourneyResultV025({required bool won, required String gameId}) async {
    ...
    if (serverConnected && api.token != null && run['local_mode'] != true) {
      try {
        final data = await api.recordChallengeJourneyResultV025(won, 'v025-${run['id']}-$gameId-${DateTime.now().microsecondsSinceEpoch}', gameId);
