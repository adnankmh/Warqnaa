...
const List<Map<String, dynamic>> luckyWheelSegmentsV182 = <Map<String, dynamic>>[
  <String, dynamic>{'key':'ticket_500','label_ar':'تذكرة 500','label_en':'Ticket 500','icon':'🎟️','weight':10,'color':'#7c3aed','reward':<String,dynamic>{'type':'ticket','value':'500','duration_hours':0,'rarity':'legendary','icon':'🎟️','label_ar':'تذكرة مسابقة 500'}},
  <String, dynamic>{'key':'pasha_day','label_ar':'يوم باشا','label_en':'Pasha Day','icon':'👑','weight':12,'color':'#dc2626','reward':<String,dynamic>{'type':'pasha_day','value':'1','duration_hours':24,'rarity':'legendary','icon':'👑','label_ar':'يوم باشا','store_item_key':'lucky_wheel_pasha_day_v182'}},
  <String, dynamic>{'key':'writing_color','label_ar':'لون كتابة','label_en':'Writing Color','icon':'✍️','weight':18,'color':'#0891b2','reward':<String,dynamic>{'type':'writing_color','value':'#22d3ee','duration_hours':24,'rarity':'rare','icon':'✍️','label_ar':'لون كتابة لمدة يوم','store_item_key':'lucky_wheel_chat_cyan_v182'}},
  <String, dynamic>{'key':'player_color','label_ar':'لون لاعب','label_en':'Player Color','icon':'🎨','weight':18,'color':'#ca8a04','reward':<String,dynamic>{'type':'player_color','value':'#facc15','duration_hours':24,'rarity':'rare','icon':'🎨','label_ar':'لون لاعب لمدة يوم','store_item_key':'lucky_wheel_name_gold_v182'}},
  <String, dynamic>{'key':'tokens_250','label_ar':'250 توكن','label_en':'250 Tokens','icon':'🪙','weight':28,'color':'#15803d','reward':<String,dynamic>{'type':'tokens','value':'250','duration_hours':0,'rarity':'common','icon':'🪙','label_ar':'250 توكن مجاني'}},
  <String, dynamic>{'key':'royal_box','label_ar':'غلاف ملكي','label_en':'Royal Cover','icon':'🎁','weight':14,'color':'#be123c','reward':<String,dynamic>{'type':'profile_cover','value':'cover_v02_royal','duration_hours':72,'rarity':'epic','icon':'🖼️','label_ar':'غلاف شخصي ملكي لمدة 3 أيام','store_item_key':'lucky_wheel_royal_cover_v182'}},
];
...
class _LuckyWheelPageV182State extends State<LuckyWheelPageV182> with SingleTickerProviderStateMixin {
  ...
  @override Widget build(BuildContext context){
    ...
    return Scaffold(appBar:AppBar(title:const Text('دولاب الحظ الملكي')),body:ListView(padding:const EdgeInsets.all(16),children:<Widget>[
      ...
      const SizedBox(height:6),const Text('النتيجة معتمدة من الخادم عند الاتصال، وتظهر بعد دوران سلس مدته 4 ثوانٍ.',textAlign:TextAlign.center,style:TextStyle(fontSize:11,height:1.5)),
      const SizedBox(height:18),
      Center(child:SizedBox(width:size,height:size+28,child:Stack(alignment:Alignment.center,children:<Widget>[
        Positioned(top:0,child:Icon(Icons.arrow_drop_down_rounded,size:62,color:Colors.amber.shade300)),
        Positioned(bottom:0,child:Transform.rotate(angle:angle,child:CustomPaint(size:Size.square(size-20),painter:LuckyWheelPainterV182()))),
        Positioned(bottom:(size-20)/2-31,child:Container(width:64,height:64,decoration:BoxDecoration(shape:BoxShape.circle,gradient:const LinearGradient(colors:<Color>[Color(0xffffe08a),Color(0xffa16207)]),border:Border.all(color:Colors.white,width:3),boxShadow:const <BoxShadow>[BoxShadow(blurRadius:16,color:Colors.black54)]),child:const Center(child:Text('W',style:TextStyle(fontWeight:FontWeight.w900,fontSize:25,color:Color(0xff3b2304))))),),
      ]))),
...
class LuckyWheelPainterV182 extends CustomPainter {
  @override void paint(Canvas canvas,Size size){
    final center=Offset(size.width/2,size.height/2);final radius=size.width/2;const sweep=math.pi*2/6;
    final fill=Paint()..style=PaintingStyle.fill;final border=Paint()..style=PaintingStyle.stroke..strokeWidth=4..color=const Color(0xffffd166);
    for(var i=0;i<6;i++){
      final start=-math.pi/2+i*sweep;fill.color=colorFromHex(luckyWheelSegmentsV182[i]['color'].toString());
      canvas.drawArc(Rect.fromCircle(center:center,radius:radius),start,sweep,true,fill);
      canvas.save();canvas.translate(center.dx,center.dy);canvas.rotate(start+sweep/2);
      final painter=TextPainter(textDirection:TextDirection.rtl,textAlign:TextAlign.center,text:TextSpan(text:'${luckyWheelSegmentsV182[i]['icon']}\n${luckyWheelSegmentsV182[i]['label_ar']}',style:const TextStyle(color:Colors.white,fontSize:12,fontWeight:FontWeight.w900,height:1.25,shadows:<Shadow>[Shadow(color:Colors.black54,blurRadius:4)])))..layout(maxWidth:radius*.58);
      painter.paint(canvas,Offset(radius*.36,-painter.height/2));canvas.restore();
    }
    canvas.drawCircle(center,radius-2,border);
    canvas.drawCircle(center,radius*.13,Paint()..color=const Color(0xfffbbf24));
  }
